...
class Uploader {
  ...
  private function initialize($field, $options)
  {
        ...
        if ($validate)
		{
            ...
            for ($i = 0; $i < count($field['name']); $i++)
			{
				...
				elseif ($field['Field_Type'] == 'link')
				{
                    ...
                    $field['name'][0] = pathinfo($tmp_name, PATHINFO_BASENAME);
                    $field['type'][0] = $link['type'];
                    $field['size'][0] = $link['size'];
                    $field['error'][0] = 0;
					
                }
				$file_info = pathinfo($field['name'][$i]);  

				//$metas['file_info'] = $file_info;
				...
				$metas['filename'] = $file_info['filename'];
                $metas['old_name'] = $file_info['basename'];
                $metas['size'] = $field['size'][$i];
                $metas['size2'] = $this->formatSize($metas['size']);
                $metas['name'] = $this->generateFileName($this->options['title'], array('name'=>$this->utf8_tis($file_info['filename']), 'size'=>$metas['size'])) . (!empty($metas['extension']) ? "." . $metas['extension'] : "");
                $metas['file'] = $this->options['uploadDir'] . $metas['name'];
                $metas['replaced'] = file_exists($metas['file']);
                $metas['date'] = date('Y-m-d H:i:s');
	...
	private function utf8_tis($string)
	{
		$str = $string;
		$res = '';
    ...
    $type = $type ? $type : 'name';
		...
		$random_string = $this->options['prefix'].substr(str_shuffle("_0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"), 0, $length ? $length : 20);
		$string = "";
